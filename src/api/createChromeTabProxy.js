...
export const createChromeTabProxy = store => {
  ...
  const dispatch = action => {
    if (typeof action !== 'object' || !action || !action.type) {
      throw new TypeError(
        'Zedux Sync - Invalid object dispatched to child store. Proxy stores only accept plain object actions. The "type" property is required.'
      )
    }

    const message = createUpMessage(action)

    // Pass the action up to the runtime
  ...
  const onChildMessage = ({ data: message }) => {
    ...
    // Forward the proxy message on to the runtime
    port.postMessage(message)
  }
  ...
  const port = chrome.runtime.connect(createConnectionId())

  port.onMessage.addListener(onParentMessage)
